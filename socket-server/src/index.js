...
function countsForRoom(room, questionIndex) {
    const map = room.answers.get(questionIndex);
    const counts = [0, 0, 0, 0];
    if (!map)
        return { counts, totalAnswers: 0 };
...
io.on("connection", (socket) => {
    ...
    socket.on("question:show", ({ pin, question }) => {
        if (!pin || !question)
            return;
        const room = getRoom(pin);
    ...
    socket.on("reveal", ({ pin, questionIndex, correctIndex }) => {
        ...
        if (room.scoredQuestions.has(qIndex))
            return;
