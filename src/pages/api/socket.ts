...
// ✅ import shared scoring (relative path avoids alias issues inside pages/api)
import { calcPoints } from "../../lib/quizScoring";
  ...
  if (!res.socket.server.io) {
    ...
    io.on("connection", (socket) => {
      ...
      socket.on("reveal", ({ pin, questionIndex, correctIndex }: { pin: string; questionIndex: number; correctIndex?: number }) => {
        ...
        // ✅ send to students immediately (even if timer not out)
        io.to(pin).emit("answer:reveal", {
          questionIndex: qIndex,
          correctIndex: correct,
          maxTime, // ✅ helps student compute exactly even after refresh
        });
      ...
      socket.on("leave", ({ pin, studentId }: { pin: string; studentId: string }) => {
        ...
        for (const map of room.answers.values()) {
