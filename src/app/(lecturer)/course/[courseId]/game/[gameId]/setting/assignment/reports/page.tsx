...
type SortKey = "createdAt" | "title" | "students" | "avgScore";
...
export default function AssignmentReportHistoryPage() {
  ...
  const [createdAtMap, setCreatedAtMap] = useState<Record<string, string>>({});

  useEffect(() => {
    let alive = true;
    (async () => {
      if (!gameId) { if (alive) setGame(null); return; }
      try {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      if (!gameId) { setRowsRaw([]); setCreatedAtMap({}); setLoading(false); return; }
      setLoading(true);
      try {
        ...
        console.error(e);
        if (!cancelled) { setRowsRaw([]); setCreatedAtMap({}); }
      } finally {
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <h3 className="text-base font-semibold text-slate-900 dark:text-slate-50">Assignment Reports</h3>
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right">
                      <Link
                        ...
                        className="inline-flex items-center justify-center gap-2 rounded-2xl px-3 py-2 text-xs font-semibold bg-white/80 text-slate-700 border border-slate-200/70 hover:bg-slate-50/70 transition dark:bg-slate-950/55 dark:text-slate-200 dark:border-slate-800/70 dark:hover:bg-slate-900/30"
                      >
